...
1. 触发条件
   ...
   - overflow不为visible
   - display为flex、grid、inline-block
2. 应用场景
   -  清除浮动
   -  防止margin重叠
   -  自适应两栏布局
...
> [见详细步骤](./afterInputUrl.md)

### 5.2 HTTP请求状态码

> HTTP状态码主要是分为5大类
 - 1xx（信息）：请求已接收，继续处理。（较少见，如 101 Switching Protocols 用于WebSocket握手）
 - 2xx（成功）：操作被成功接收并处理。
 - 3xx（重定向）：需要进一步操作以完成请求。
 - 4xx（客户端错误）：请求包含错误或无法被处理。（重点排查对象）
 - 5xx（服务端错误）：服务器处理请求出错。（背锅对象）
...
<body>
    <div class="parent">
        ...
        <div class="left-side">Left Sidebar (固定宽200px)</div>
        <main>Main Content (自适应宽度)</main>
        <div class="right-side">Right Sidebar (固定宽200px)</div>
